  ...
  const fetchRetailerDetails = async () => {
    try {
      ...
      if (json?.success && json?.data) {
        ...
        if (data.pancard) {
          setPanImage({ uri: data.pancard });
          setPanLocked(true); // 🔒 lock PAN editing
        }

        if (data.aadhar_card) {
          setAadharImage({ uri: data.aadhar_card });
          setAadharLocked(true); // 🔒 lock Aadhar editing
        }
      }
    } catch (error) {
      console.log(error);
      ...
      >
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <UploadImageBtn
            label="PAN Card"
            image={panImage}
            onChange={panLocked ? null : setPanImage}
            disabled={panLocked}
          />
          {panLocked && (
            <Text style={styles.lockedText}>
              PAN cannot be changed once uploaded
            </Text>
          )}
          ...
          <UploadImageBtn
            label="Aadhar Card"
            image={aadharImage}
            onChange={aadharLocked ? null : setAadharImage}
            disabled={aadharLocked}
          />
          {aadharLocked && (
            <Text style={styles.lockedText}>
              Aadhar cannot be changed once uploaded
            </Text>
          )}
